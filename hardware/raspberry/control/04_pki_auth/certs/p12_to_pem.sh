...
CERTSDIR=${basename}
mkdir certs_issued
mkdir $(pwd)/certs_issued/$CERTSDIR
...
openssl x509 -in ${basename}.cert.pem -noout -ext subjectAltName
...
mv ${basename}.* $(pwd)/certs_issued/$CERTSDIR/
mv ${file_path} $(pwd)/certs_issued/$CERTSDIR/
mv ca-chain.* $(pwd)/certs_issued/$CERTSDIR/
